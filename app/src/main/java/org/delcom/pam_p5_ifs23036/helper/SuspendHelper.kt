...
object SuspendHelper {
    ...
    suspend fun <T> safeApiCall(apiCall: suspend () -> ResponseMessage<T?>): ResponseMessage<T?> {
        return try {
            ...
            // Backend Ktor mengembalikan ErrorResponse saat terjadi error 4xx/5xx
            val jsonError = try {
                Gson().fromJson(errorResponse, ResponseMessage::class.java)
            } catch (ex: Exception) {
                null
            }
            ...
            ResponseMessage(
                status = "error",
                message = jsonError?.message ?: "Terjadi kesalahan pada server (Error ${e.code()})"
            )
        } catch (e: Exception) {
            e.printStackTrace()
            ResponseMessage(
                status = "error",
                message = e.message ?: "Koneksi ke server gagal"
            )
        }
    }
}
